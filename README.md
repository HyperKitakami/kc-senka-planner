...
当前版本：**`v0.3.1`** —— 新增「预设 EX 任务」卡片与 poi 季常同步。
...
* `v0.3.1` —— 新增内置「预设 EX 任务」（季常 7 项 / 年常 2 项），在任务页 EO 卡片下方
  单独成卡片；接入 poi 季常完成状态批量同步（仅季常，年常手动勾选）；任务名称列改为
  显示游戏内完整任务名（简称保留为悬停提示）；内置任务支持升级时补齐新增的展示字段。
